...
def test_action_mapping():
    ...
    for i, action in enumerate(test_actions):
        ...
        # 映射动作到规划参数
        delta_d0 = config["alpha_1"] * action[0]
        delta_dT = config["alpha_2"] * action[1]
        vT = config["v_min"] + (action[2] + 1) / 2 * (config["v_max"] - config["v_min"])
        T = config["T_min"] + (action[3] + 1) / 2 * (config["T_max"] - config["T_min"])
        
        print(f"  起点横向偏移校正: {delta_d0:.2f} m")
    ...
    try:
        ...
        for step in range(max_steps):
            ...
            if action_policy == 'random':
                ...
                action = [0.0, -1.0, 0.0, -0.5]
            elif action_policy == 'custom':
                # 自定义策略：轻微的随机扰动
                action = [np.random.uniform(-0.2, 0.2) for _ in range(4)]
            else:
...
def run_multiple_episodes(num_episodes=10, max_steps=500, action_policy='random', render_episodes=None):
    ...
    config = {
        ...
        "T_min": 2.0,
        "T_max": 8.0,
        "use_extended_reference_line": False
    }
        ...
        test_configs = [
            {
                ...
                'max_steps': 500,
                'action_policy': 'zero',
                'render_episodes': None  # 只渲染第1个episode
            },
            {
                'name': '随机动作策略测试',
                'num_episodes': 0,
