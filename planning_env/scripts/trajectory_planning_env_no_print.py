...
try:
    ...
    print("Warning: cubic_spline_planner not found, using simplified spline interpolation")
    cubic_spline_planner = None

# 导入LQR控制相关函数
try:
    from lqr_steer_control import lqr_steering_control, State, pi_2_pi, calc_nearest_index
    LQR_AVAILABLE = True
except ImportError as e:
    print(f"Warning: lqr_steer_control not available: {e}")
    LQR_AVAILABLE = False
...
class TrajectoryPlanningEnv(MetaDriveEnv):
    ...
    def __init__(self, config=None):
        ...
        # 统一的调试打印频率控制
        self.debug_print_frequency = 1  # 每10步打印一次调试信息
        self.error_print_frequency = 50  # 每50步打印一次错误信息
        
        # LQR控制参数
        self.lqr_Q = np.eye(4) * 1  # 状态权重矩阵
        self.lqr_R = np.eye(1) * 1  # 控制权重矩阵
        self.lqr_dt = 0.1  # LQR控制时间步长
        self.lqr_L = 2.5   # 车辆轴距 (m)
        self.lqr_max_steer = np.deg2rad(45.0)  # 最大转向角 (rad)
        self.lqr_Kp = 1.0  # 速度PID比例增益
        
        # LQR控制状态变量
        self.lqr_pe = 0.0    # 上一步的横向误差
        self.lqr_pth_e = 0.0 # 上一步的航向误差
    ...
    def observation_space(self):
        ...
        使用MetaDrive默认的观测空间
        """
        # 使用父类MetaDriveEnv的默认观测空间
        return super().observation_space
    
    def _get_observation(self):
        """
        使用MetaDrive默认观测方法获取观测向量
        
        Returns:
            np.ndarray: MetaDrive默认观测向量
        """
        # 使用MetaDrive默认的观测方法
    ...
    def _preprocess_trajectory(self, trajectory):
        """对轨迹进行预处理，生成样条插值的连续路径点
        Args:
            trajectory: 轨迹点列表 [(x, y, heading, velocity), ...]
        Returns:
            tuple: (cx, cy, cyaw, ck, speed_profile) 插值后的路径点
        """
        if len(trajectory) < 2:
            return [], [], [], [], []
        
        # 提取轨迹点坐标和速度
        ax = [point[0] for point in trajectory]
        ay = [point[1] for point in trajectory]
        av = [point[3] for point in trajectory]  # 速度
        
        # 使用样条插值生成连续路径
        if cubic_spline_planner is not None:
            try:
                cx, cy, cyaw, ck, s = cubic_spline_planner.calc_spline_course(
                    ax, ay, ds=0.1)  # 0.1m间隔插值
                ...
                if len(s) > 0:
                    ...
                    for i in range(1, len(ax)):
                        dist = math.sqrt((ax[i] - ax[i-1])**2 + (ay[i] - ay[i-1])**2)
                        original_s.append(original_s[-1] + dist)
                    
                    # 对速度进行插值
                    speed_profile = np.interp(s, original_s, av).tolist()
                else:
                    speed_profile = av
                    
                return cx, cy, cyaw, ck, speed_profile
            except Exception as e:
                if self.step_count % self.error_print_frequency == 0:
                    print(f"样条插值失败，使用简化插值: {e}")
    ...
    def _get_target_trajectory_point(self):
        ...
        #     print(f"前瞻距离: {unified_lookahead_distance:.2f},总点数:{len(self.current_trajectory) - 1} 目标索引: {target_index}")
    ...
    def step(self, actions):
        ...
        if self.ros_publisher is not None:
            try:
                ...
                # 更新导航点数据
                if hasattr(self, 'agent') and self.agent is not None:
                    self.ros_publisher.update_navigation_data(self.agent)
                
                # 发布车辆状态信息（转角和油门刹车量）
                if hasattr(self, 'agent') and self.agent is not None:
                    # 获取转角信息（度）
                    steering_deg = steering * 60.0  # 将归一化的转向值转换为度数
                    # 获取油门刹车量（百分比）
                    throttle_brake_percent = abs(throttle_brake) * 100.0  # 转换为百分比
                    # 获取车辆位置和航向角
                    vehicle_position = [self.agent.position[0], self.agent.position[1]]
                    vehicle_heading = self.agent.heading_theta
                    
                    # 发布速度信息（跟随车辆移动）
                    current_velocity = self.agent.speed
                    target_velocity = getattr(self, 'target_velocity', current_velocity)
                    self.ros_publisher.update_velocity_markers(
                        current_velocity, target_velocity, vehicle_position, vehicle_heading
                    )
                    
                    # 发布车辆状态显示
                    self.ros_publisher.update_vehicle_status_display(
                        steering_deg, throttle_brake_percent, vehicle_position, vehicle_heading
                    )
        ...
        custom_reward, step_inf = self._get_reward(trajectory, planning_params)
    ...
    def _trajectory_tracking_control(self, target_point):
        """
        轨迹跟踪控制算法 - LQR控制器（使用导入的lqr_steering_control函数）
        Args:
        ...
        if not LQR_AVAILABLE:
            # 如果LQR控制不可用，回退到简单控制
            target_velocity = target_point[3]
            current_velocity = self.agent.speed
            velocity_error = target_velocity - current_velocity
            throttle_brake = np.clip(self.lqr_Kp * velocity_error, -1.0, 1.0)
            return 0.0, throttle_brake
        ...
        # 对轨迹进行预处理，生成样条插值的连续路径点
        # FIXME 注意，此处速度也有插值，但是目标速度应该用哪一个？--目前使用预瞄距离的那个速度
        cx, cy, cyaw, ck, speed_profile = self._preprocess_trajectory(self.current_trajectory)
        ...
        # 创建State对象以适配lqr_steering_control接口
        state = State(x=current_x, y=current_y, yaw=current_heading, v=current_velocity)
        
        # 使用导入的LQR转向控制函数
        delta, target_ind, self.lqr_pe, self.lqr_pth_e = lqr_steering_control(
            state, cx, cy, cyaw, ck, self.lqr_pe, self.lqr_pth_e, self.dt)
        
        # 转向角归一化到[-1, 1]
        max_steering_deg = 60.0  # 最大转向角度
        steering_deg = delta * 180.0 / np.pi
        steering = np.clip(steering_deg / max_steering_deg, -1.0, 1.0)
        
        # 速度控制 - 使用PID控制
        # 目标速度
        # if target_ind < len(speed_profile):
        #     target_v = speed_profile[target_ind]
        #     print("1", target_v)
        ...
        velocity_error = target_v - current_velocity
        throttle_brake = np.clip(self.lqr_Kp * velocity_error, -1.0, 1.0)
        ...
        #     print(f"  当前航向: {current_heading:.3f}")
        #     print(f"  目标索引: {target_ind}")
        #     print(f"  横向误差: {self.lqr_pe:.3f}")
        #     print(f"  航向误差: {self.lqr_pth_e:.3f}")
        #     print(f"  转向角: {delta:.3f} rad ({steering_deg:.1f} deg), steering_deg/60:{steering_deg/60}")
        #     # print(f"steering: {steering}")
            # print(f"step: {self.step_count}")
            # print(f"  目标速度: {target_v:.2f}, 当前速度: {current_velocity:.2f}")
            # print("*"*20)
        ...
        if hasattr(self, 'ros_publisher') and self.ros_publisher is not None:
            try:
                # 更新速度信息 发布float话题
                self.ros_publisher.update_velocity(target_v, current_velocity)
                
                vehicle_position = [self.agent.position[0], self.agent.position[1]]
                vehicle_heading = self.agent.heading_theta
                
                # 更新速度可视化标记 发布marker
                # vehicle_position = [current_x, current_y]
                self.ros_publisher.update_velocity_markers(current_velocity, target_v, vehicle_position, vehicle_heading)
                
                # 发布LQR处理后的轨迹（包含yaw、cx、cy信息）
                if len(cx) > 0 and len(cy) > 0 and len(cyaw) > 0:
                    lqr_trajectory_points = [(cx[i], cy[i], cyaw[i]) for i in range(len(cx))]
                    self.ros_publisher.update_lqr_trajectory(lqr_trajectory_points)
                    
            except Exception as e:
                if self.step_count % self.error_print_frequency == 0:
                    print(f"Failed to publish velocity/LQR data: {e}")
    ...
    def _action_to_planning_params(self, actions):
        ...
        # print("alpha_2: ", self.config["alpha_2"], "a2: ", a2, "delta_dT: ", delta_dT)
        # 终端目标速度
    ...
    def _is_arrive_checkpoint(self, vehicle_position, check_point, threshold):
        """
        Returns True only the *first time* the vehicle reaches the current checkpoint.
        Automatically resets when checkpoint changes.
        """

        vehicle_xy = np.array(vehicle_position)
        check_point = np.array(check_point)

        # ================================
        # ① 如果是新的 checkpoint → 重置状态
        # ================================
        if not hasattr(self, "_last_checkpoint"):
            self._last_checkpoint = check_point.copy()
            self._current_cp_reached = False

        # 如果 checkpoint 变了（说明 navigation 已切换到下一个）
        if np.linalg.norm(check_point - self._last_checkpoint) > 1e-6:
            # 自动重置
            self._current_cp_reached = False
            self._last_checkpoint = check_point.copy()
            # print("[Checkpoint] new checkpoint detected, reset flag.")

        # 如果已经到达过当前 checkpoint，就不再触发
        if self._current_cp_reached:
            return False

        # ================================
        # ② 判断是否到达 checkpoint
        # ================================
        dist = np.linalg.norm(vehicle_xy - check_point)
        # print("distance:", dist)

        if dist <= threshold:
            print(f"[Checkpoint] reached! dist={dist:.3f}")
            self._current_cp_reached = True
            return True
    ...
    def _get_reward(self, trajectory, planning_params):
        ...
        使用MetaDrive默认奖励函数逻辑
        Args:
            trajectory: 生成的轨迹点列表（保持接口兼容性）
            planning_params: 规划参数（保持接口兼容性）
        Returns:
            float: 奖励值
        """
        ...
        # print(dir(vehicle))
        # print( )
        # print(dir(vehicle.navigation._get_info_for_checkpoint))
        # print("vehicle.navigation._get_info_for_checkpoint",vehicle.navigation._get_info_for_checkpoint)
        # info = vehicle.navigation._get_info_for_checkpoint()
        if vehicle.lane in vehicle.navigation.current_ref_lanes:
            current_ref_lanes = vehicle.lane
            # print('current_ref_lanes', current_ref_lanes.index)

        info = vehicle.navigation._get_info_for_checkpoint(
            # lanes_id=current_ref_lanes.index[2],
            lanes_id=0,
            ref_lane=vehicle.navigation.current_ref_lanes[0],
            # ref_lane=current_ref_lanes,
            ego_vehicle=vehicle
        )
        # print("info:", info)

        check_point = info[2] # maybe np.array
        # info[2] 全局地卡尔坐标下的navigation (check point) x and y, 从车道０开始
        ...
        if self.config.get("use_lateral_reward", True):
            lateral_factor = max(0.0, min(1.0, 1 - 2 * abs(lateral_now) / vehicle.navigation.get_current_lane_width()))
        else:
            lateral_factor = 1.0

        reward = 0.0
        reward += self.config.get("driving_reward", 0.25) * (long_now - long_last) * lateral_factor * positive_road
        reward += self.config.get("speed_reward", 0.1) * (vehicle.speed_km_h / vehicle.max_speed_km_h) * positive_road

        # Terminal rewards/penalties
        if self._is_arrive_destination(vehicle):
            reward = +self.config.get("success_reward", 10.0)
            print("success")
        elif self._is_out_of_road(vehicle):
            reward = -self.config.get("out_of_road_penalty", 5.0)
        elif vehicle.crash_vehicle:
            reward = -self.config.get("crash_vehicle_penalty", 5.0)
        elif vehicle.crash_object:
            reward = -self.config.get("crash_object_penalty", 5.0)
        elif vehicle.crash_sidewalk:
            reward = -self.config.get("crash_sidewalk_penalty", 1.0)
        elif self._is_arrive_checkpoint(vehicle_pos,check_point,1.6):
            reward = +3
            print("_is_arrive_checkpoint")

        step_info["route_completion"] = vehicle.navigation.route_completion

        # print("reward info: ", 
        #       self.config.get("driving_reward", 0.25) * (long_now - long_last) * lateral_factor * positive_road, 
        #       self.config.get("speed_reward", 0.1) * (vehicle.speed_km_h / vehicle.max_speed_km_h) * positive_road)
...
class TrajectoryPlanner:
    ...
    def generate_trajectory(self, vehicle, planning_params):
        ...
        # if self.step_count % self.debug_print_frequency == 0:
            ...
            # print(f"[TrajectoryPlanner] lateral_traj: {lateral_traj[-1]}")
    ...
    def _generate_lateral_trajectory(self, current_state, planning_params):
        ...
        # print("dT: ", dT, "current_state['d0']", current_state['d0'], "planning_params['delta_dT']", planning_params['delta_dT'])
        dd_dtT = 0.0  # 终端横向速度为0
