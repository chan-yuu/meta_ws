...
class TrajectoryPlanningROSPublisher:
    ...
    def __init__(self, node_name="trajectory_planning_publisher", publish_rate=10.0):
        """
        初始化ROS发布器
    ...
    def _init_ros(self):
        ...
        try:
            ...
            # 创建发布器
            self.odom_pub = rospy.Publisher('/vehicle/odometry', Odometry, queue_size=10)
            self.path_pub = rospy.Publisher('/vehicle/trajectory', Path, queue_size=10)
            self.marker_pub = rospy.Publisher('/environment/markers', MarkerArray, queue_size=10)
            self.reference_path_pub = rospy.Publisher('/trajectory/reference_path', Path, queue_size=10)
            self.target_vel = rospy.Publisher('/target_vel', Float64, queue_size=10)
            self.current_vel = rospy.Publisher('/current_vel', Float64, queue_size=10)
            # 创建TF广播器
            self.tf_broadcaster = tf2_ros.TransformBroadcaster()
            
            rospy.loginfo(f"ROS Publisher initialized: {self.node_name}")
            ...
            try:
                with self.data_lock:
                    ...
                    if self.target_v is not None and self.current_velocity is not None:
                        # debug
                        # print("self.target_v: ", self.target_v)
